    ...
    def read_all_values_from_file(filename):
        with open(filename, 'rb') as infp:
            ...
            if not data:
                ...
                # _INITIAL_MMAP_SIZE afterwards, so a reader can observe it
                # while it is still empty. A process dying in that window can
                # also leave an empty file behind for good. Treat it as a file
                # with nothing recorded in it yet, mirroring the capacity == 0
                # case in __init__, rather than failing to unpack a header that
                # isn't there. Anything longer than this is left to fail loudly.
                return iter(())
            used = _unpack_integer(data, 0)[0]
