...
from ..validation import validate_metric_name_token, validate_metric_label_name_token
    ...
    for char in text:
        if state == 'startoflabelname':
            ...
            if char == '\\':
                ...
                ln = ''.join(labelname)
                if not validate_metric_label_name_token(ln):
...
def _parse_labels(text):
    ...
    try:
        ...
        while sub_labels:
            ...
            if not validate_metric_label_name_token(label_name):
                raise ValueError("invalid line, bad label name: " + text)
...
def text_fd_to_metric_families(fd):
    ...
    def build_metric(name, documentation, typ, unit, samples):
        ...
        if not validate_metric_name_token(name):
