...
from prometheus_client import metric_wrappers
...
from prometheus_client.value import _MutexValue


class TestMultiProcess(unittest.TestCase):

    def _set_value_class(self, new_value_class):
        metric_wrappers._ValueClass = new_value_class

    def setUp(self):
        self.tempdir = tempfile.mkdtemp()
        os.environ['prometheus_multiproc_dir'] = self.tempdir
        self._set_value_class(_MultiProcessValue(lambda: 123))
    ...
    def tearDown(self):
        ...
        self._set_value_class(_MutexValue)

    def test_counter_adds(self):
        c1 = Counter('c', 'help', registry=None)
        self._set_value_class(_MultiProcessValue(lambda: 456))
    ...
    def test_counter_across_forks(self):
        pid = 0
        self._set_value_class(_MultiProcessValue(lambda: pid))
        c1 = Counter('c', 'help', registry=None)
        self.assertEqual(0, self.registry.get_sample_value('c_total'))
        c1.inc(1)
        c1.inc(1)
